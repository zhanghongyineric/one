      ...
      >
        ...
        <el-table-column label="操作" min-width="90" align="center">
          <template slot-scope="scope">
            <BreakInfo :index="scope.$index" :list="tableList" :only-base="true" />
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    getList() {
      ...
      getSearchList({ ...this.search, ...this.listQuery }, 'notDetectedInfo')
        .then(res => {
    ...
    download() {
      this.loading = true
      download(this.$route.query.search, 'notDetectedXlsDownload')
        .then(_ => {
          this.loading = false
        }).catch(_ => {
          this.loading = false
    ...
    showDetail(scope) {
      ...
      this.breakInfo = this.tableList[this.currIndex]
