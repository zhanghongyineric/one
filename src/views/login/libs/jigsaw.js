...
let tryTimes = 1 // 图片加载失败时已尝试的加载次数
...
function createImg(onload) {
  ...
  img.onerror = () => {
    if (tryTimes > 3) {
      Notification({ duration: 0, type: 'error', message: '非常抱歉,拼图图片加载失败了!这会让你无法登录,你可以尝试刷新页面或者联系管理员' })
    }
    // 图片加载失败的时候重新加载其他图片
    if (tryTimes <= 3) {
      tryTimes++
...
function getRandomImgSrc() {
  const random = parseInt(Math.random() * 500 + 1)

  return `${process.env.VUE_APP_IMAGES_URL}/slider_img_${random}.jpg`
}
...
class Jigsaw {
  constructor({ el, width = w, height = h, onSuccess, onFail, onRefresh }) {
    ...
    this.width = width
    this.height = height
    this.status = false // 拼图当前的成功状态，true成功，false失败
    this.el = el
  ...
  bindEvents() {
    ...
    const handleDragStart = function(e) {
      if (_this.status || _this.isLoading) return // 如果当前拼图状态为成功,则不允许在滑动
