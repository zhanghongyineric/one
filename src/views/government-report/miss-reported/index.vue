  ...
  <div class="work-card layout-content demo-page">
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column prop="logId" label="日志编号" min-width="150" />
        <el-table-column prop="reportTime" label="报备时间" min-width="170" />
        <el-table-column prop="violationTime" label="违章日期" min-width="100" />
        <el-table-column label="漏报违章数" min-width="100">
          ...
          <template slot-scope="scope">
            <router-link
              ...
              :to="{query: {...scope.row, ...{search: search}},
                    path: '/government-report/miss-reported/:' + scope.row.logId}"
            >查看漏报记录</router-link>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    getList() {
      ...
      getSearchList({ ...this.search, ...this.listQuery }, 'selectMissReportRecentDays')
        .then(res => {
    ...
    download() {
      this.loading = true
      download({ flag: this.search.flag }, 'missReportXlsDownload')
        .then(_ => {
          this.loading = false
        }).catch(_ => {
          this.loading = false
