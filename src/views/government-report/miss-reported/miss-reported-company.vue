    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column prop="typeName" label="营运类型" min-width="120">
          ...
          <!--  {{ transportType[scope.row.typeId] }}-->
          <!--</template>-->
        </el-table-column>
        <el-table-column prop="driverName" label="驾驶员" min-width="80" />
        <el-table-column prop="typeName" label="违章类型" min-width="120">
          <template slot-scope="scope">
...
export default {
  ...
  created() {
    ...
    this.search.violationTime = this.$route.query.violationTime
    this.getList()
  },
  methods: {
    getList() {
      this.listLoading = true
      getSearchList({ ...this.search, ...this.listQuery }, 'selectMissReportList')
        .then(res => {
    ...
    download() {
      this.loading = true
      download(this.$route.query.search, 'missReportXlsDownload')
        .then(_ => {
          this.loading = false
        }).catch(_ => {
          this.loading = false
