      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <SetMarkers
              v-if="statusList[scope.row.status] === '已处理' && scope.row.tagFlag === '0'"
              :id="scope.row.logId"
              @refresh="getList"
            />
            <IndustryMarkers
              v-if="statusList[scope.row.status] === '已处理' && scope.row.tagFlag === '1'"
...
import { getSearchList, download } from '@/api/government-report/government-report.js'
...
export default {
  ...
  created() {
    ...
    this.search.reportTime = this.$route.query.reportTime.substr(0, 10)
    this.getList()
  },
  methods: {
    getList() {
      this.listLoading = true
      getSearchList({ ...this.search, ...this.listQuery }, 'selectNoEndInfo')
        .then(res => {
    ...
    download() {
      this.loading = true
      download(this.$route.query.search, 'noEndReporttXlsDownload')
        .then(_ => {
          this.loading = false
        }).catch(_ => {
          this.loading = false
