  ...
  <div class="work-card layout-content demo-page">
    <el-card class="box-card">
      ...
      <el-form class="table-page-search-wrapper search" label-width="60px" size="small">
        <el-row :gutter="48">
          <el-col :md="7" :sm="22" :xs="22" style="padding-right: 10px">
      ...
      >
        ...
        <el-table-column prop="evaluation" label="超时" min-width="170" />
        <el-table-column label="操作" width="140" align="center">
          <template slot-scope="scope">
            <router-link
              exact
              :to="{query: {...scope.row, ...{search: search}},
                    path: '/government-report/reporting/:' + scope.row.logId}"
            >详情</router-link>
          </template>
        </el-table-column>
      </el-table>
...
import { getSearchList, download, getMarkersList } from '@/api/government-report/government-report.js'
...
export default {
  ...
  created() {
    ...
    this.daysList = onlineOption.not_end_days.list
  ...
  methods: {
    getList() {
      ...
      getSearchList({ ...this.search, ...this.listQuery }, 'selectNoEnd')
        .then(res => {
    ...
    download() {
      this.loading = true
      download({ flag: this.search.flag }, 'noEndReporttXlsDownload')
        .then(_ => {
          this.loading = false
        }).catch(_ => {
          this.loading = false
