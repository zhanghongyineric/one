  ...
  <div class="work-card layout-content demo-page">
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column prop="noReportTime" label="未报备日期" min-width="120" />
        <el-table-column prop="violationTime" label="违章日期" min-width="120" />
        <el-table-column label="操作" width="140" align="center">
          <template slot-scope="scope">
            <router-link
              exact
              :to="{query: {...scope.row, ...{search: search}},
                    path: '/government-report/not-reported/:' + scope.row.logId}"
            >查看违章记录</router-link>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    getList() {
      ...
      getSearchList({ ...this.search, ...this.listQuery }, 'selectNoReportRecentDays')
        .then(res => {
    ...
    download() {
      this.loading = true
      download({ flag: this.search.flag }, 'noReportXlsDownload')
        .then(_ => {
          this.loading = false
        }).catch(_ => {
          this.loading = false
